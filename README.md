...
**Pin what matters. Explore agent-assisted arrangements.**
...
No backend, no login, no API key. State is restored from `sessionStorage` in
the current tab. Export or import versioned JSON, save a PNG, and undo or redo
room commands when you want to explore safely.
...
**How it is implemented.** `document.modelContext.registerTool` exposes a stable
read/rule set. Selection, pins, and open violations abort the previous
`AbortController` and register the tools that make sense now. Writes go through
`executeCommand()` in `web/lib/places/commands.ts`. If a move hits a pin, the
command returns `pinned_by_human` and the sprite does not move.
...
| `list_constraints` | Read the rules enforced by the solver |
| `add_constraint` / `remove_constraint` | Change the room's rule data |
...
Use `?seed=0` through `?seed=3` for a shareable opening challenge. Each seed has
its own tab-local saved snapshot, so reloading keeps later moves. JSON exports
use `schemaVersion: 1` and can be restored with **Import**. Undo/redo is available
from the header or with <kbd>Ctrl/⌘ Z</kbd> and <kbd>Ctrl/⌘ Shift Z</kbd>.
